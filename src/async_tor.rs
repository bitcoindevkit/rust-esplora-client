...
impl AsyncTorClient {
    ...
    // async fn hyper_request() -> {todo!()}
    // async fn hyper_bootstrap() -> {todo!()}
    // async fn get_request() -> {todo!()}
    // async fn post_request() -> {todo!()}

    async fn get_request(&self, uri: &Uri) -> Result<Request<Empty<Bytes>>, Error> {
        let mut request = Request::get(uri.path())
            .header(http::header::HOST, uri.host().ok_or(Error::InvalidUri)?)
            .body(Empty::new())?;
        ...
        if !self.headers.is_empty() {
            for (key, val) in &self.headers {
                let header_name: HeaderName =
                    HeaderName::from_str(key).map_err(|e| Error::Http(e.into()))?;
                let header_value: HeaderValue =
                    HeaderValue::from_str(val).map_err(|e| Error::Http(e.into()))?;
    ...
    async fn post_request(&self, uri: &Uri, body: Bytes) -> Result<Request<Full<Bytes>>, Error> {
        let mut request = Request::post(uri.path())
            .header(http::header::HOST, uri.host().ok_or(Error::InvalidUri)?)
    ...
    async fn send_empty(
        ...
        let uri = hyper::Uri::from_str(&self.url).map_err(|e| Error::Http(e.into()))?;
        let host = uri.host().ok_or(Error::InvalidUri)?.to_owned();
    ...
    async fn send_full(&self, request: Request<Full<Bytes>>) -> Result<Response<Incoming>, Error> {
        let uri = hyper::Uri::from_str(&self.url).map_err(|e| Error::Http(e.into()))?;
        let host = uri.host().ok_or(Error::InvalidUri)?.to_owned();
    ...
    /// Make an HTTP GET request to given URL, deserializing to `Option<T>`.
    ///
    /// It uses [`AsyncTorClient::get_response_json`] internally.
    ///
    /// See [`AsyncTorClient::get_response_json`] above for full
    /// documentation.
    ...
    /// Make an HTTP GET request to given URL, deserializing to `Option<T>`.
    ///
    /// It uses [`AsyncTorClient::get_response_hex`] internally.
    ///
    /// See [`AsyncTorClient::get_response_hex`] above for full
    /// documentation.
    ...
    /// Make an HTTP GET request to given URL, deserializing to `Option<T>`.
    ///
    /// It uses [`AsyncTorClient::get_response_text`] internally.
    ///
    /// See [`AsyncTorClient::get_response_text`] above for full
    /// documentation.
    ...
    async fn post_request_hex<T: Encodable>(&self, path: &str, body: T) -> Result<(), Error> {
        ...
        let response = self.post(&url, Bytes::from(body)).await?;
